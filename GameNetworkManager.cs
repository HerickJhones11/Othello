...
{
    public static GameNetworkManager Instance { get; private set; } // Singleton
    ...
    public int port = 12345;              // Porta de comunica��o
    private bool acceptingClients = true; // Controle de execu��o do loop de aceita��o de clientes
    ...
    // M�todo ass�ncrono para enviar uma mensagem e aguardar a resposta
    ...
    // Chamado quando os dados s�o recebidos
    ...
    public bool TryConnectAsClient()
    {
        try
        {
            ...
            StartReceivingData(); // Inicia a recep��o de dados
            isClient = true;
            color = Player.White;
            if (isServer)
            {
    ...
    private void HandleReceivedData(byte[] buffer, int bytesRead)
    {
        ...
        if (responseTasks.TryGetValue(response.requestId, out var taskCompletionSource))
        {
            ...
            responseTasks.Remove(response.requestId);  // Remove a Task do dicion�rio
        }
        else
        {
    ...
    {
        // Procura pelo cliente oponente (assumindo que voc� tem apenas dois clientes)
    ...
    public void SendPlayerData(PlayerData playerData)
    {
        ...
        // Envia os dados pela rede (supondo que voc� tenha uma conex�o de rede estabelecida)
        if (stream != null && stream.CanWrite)
        {
            stream.Write(data, 0, data.Length);
